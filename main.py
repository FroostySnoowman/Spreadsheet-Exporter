...
def stats():
    ...
    noA = [{"number":c.number,"dateMillis":c.date_millis,"durationSecs":c.duration_secs}
           for c in calls if c.type==3]
    noS = [{"number":c.number,"dateMillis":c.date_millis,"durationSecs":c.duration_secs}
...
async def export_spreadsheet(spreadsheet_id, export_mode, column_name, txt_file_name, export_format, file_name):
    ...
    if export_mode=="single_column":
        ...
        if column_name in df.columns:
            ...
            print(f"Exported column {column_name} → {path}")
        else:
            print(f"Column '{column_name}' missing")
            return None
    else:
        ...
        print(f"Exported full sheet → {path}")
...
Thread(target=lambda: asyncio.run(run_every_hour()),
       daemon=True).start()
